...
class MultiSwapStore {
  ...
  calcPriceInfo(tokenIn, tokenOut, swap) {
    ...
    const returnAmount = ethers.utils
      .formatUnits(swap.returnAmount, tokenOut.decimals)
      .toString();
    const swapAmount = ethers.utils
      .formatUnits(swap.swapAmount, tokenIn.decimals)
      .toString();
      ...
      try {
        ...
        if (!isNetworkToken(fromAsset.address)) {
          ...
          if (BigNumber(allowance).lt(fromAmount)) {
            await emitStatus(
              emitter,
              allowanceTXID,
              `Allow the router to spend your ${fromAsset.symbol}`
            );
          } else {
            await emitNotificationDone(
              emitter,
              allowanceTXID,
              `Allowance on ${fromAsset.symbol} sufficient`
            );
          }
        } else {
        ...
        if (BigNumber(allowance).lt(fromAmount)) {
          console.log(
            "GASSSS",
            web3.utils.toWei(
              BigNumber(await stores.accountStore.getGasPrice())
                .times(GAS_MULTIPLIER)
                .toFixed(0),
              "gwei"
            ),
            (await stores.accountStore.getGasPrice()).toString()
          );
          const res = await approve(
            this.tokenIn,
            this.provider,
            multiSwapAddress,
            web3.utils.toWei(await stores.accountStore.getGasPrice(), "gwei"),
        ...
        await stores.stableSwapStore._refreshAssetBalance(
          web3,
          account,
          this.swap?.tokenIn
        );
  ...
  parseError(err) {
    if (err?.message?.indexOf("MSAmountOutLessThanRequired") !== -1) {
      ...
      err?.message?.indexOf("User denied transaction signature") !== -1
    ) {
      return null;
    } else {
  ...
  get isWrapUnwrap() {
    return (
      (this.tokenIn === FTM_SYMBOL &&
        "".concat(this.tokenOut).toLowerCase() ===
          WFTM_ADDRESS.toLowerCase()) ||
      ("".concat(this.tokenIn).toLowerCase() === WFTM_ADDRESS.toLowerCase() &&
        this.tokenOut === FTM_SYMBOL)
    );
  }

  get isDirectRoute() {
    return (
      (!!DIRECT_SWAP_ROUTES[this.tokenIn?.toLowerCase()] &&
        DIRECT_SWAP_ROUTES[this.tokenIn?.toLowerCase()] ===
          this.tokenOut?.toLowerCase()) ||
      (!!DIRECT_SWAP_ROUTES[this.tokenOut?.toLowerCase()] &&
        DIRECT_SWAP_ROUTES[this.tokenOut?.toLowerCase()] ===
          this.tokenIn?.toLowerCase())
    );
  }
  ...
  get isMultiswapInclude() {
    ...
    return !(
      MULTISWAP_INCLUDE.includes(this.tokenIn?.toLowerCase()) ||
      MULTISWAP_INCLUDE.includes(this.tokenOut?.toLowerCase())
    );
    // return true
  }
      ...
      return (
        this.swap.swaps
          ?.map((s) => {
            ...
            const dex = {
              name: this.swap.swapPlatforms
                ? this.swap.swapPlatforms[s.poolId]
                : "Sterling",
            };
