...
function Setup() {
        ...
        if (type === "From") {
            if (value.address === toAssetValue.address) {
                ...
                if (
                    ...
                    const baseAsset = stores.stableSwapStore.getStore("baseAssets");
                    const sterlingIndex = baseAsset.findIndex((token) => {
                        return token.address.toLowerCase() === DIRECT_SWAP_ROUTES[value.address].toLowerCase()
                    });
                    setToAssetValue(baseAsset[sterlingIndex]);
                    multiSwapStore.setTokenOut(baseAsset[sterlingIndex].address)
                    to = baseAsset[sterlingIndex].address
                }
            ...
            if (value.address === fromAssetValue.address) {
                ...
                if (
                    ...
                    setFromAssetValue(baseAsset[sterlingIndex]);
                    multiSwapStore.setTokenIn(baseAsset[sterlingIndex].address)
                    from = baseAsset[sterlingIndex].address
                }
    ...
    const swapIconHover = () => {
        setSwapIconBgColor(appTheme === "dark" ? "#412d2d" : "#e49b9b");
