  ...
  const ssUpdated = async () => {
    ...
    if (router.query.address && router.query.address !== "create") {
      ...
      if (storeAssetOptions.length > 0 && asset1 == null) {
        const sterlingIndex = storeAssetOptions.findIndex((token) => {
          return token.id == DEFAULT_ASSET_TO;
        });
        setAsset1(storeAssetOptions[sterlingIndex]);
        aa1 = storeAssetOptions[sterlingIndex];
      }
        ...
        {withdrawAction !== action && (
          ...
          >
            <path
              ...
              stroke={appTheme === "dark" ? "#e64c4c" : "#8e0b0b"}
            />
          </svg>
        )}
